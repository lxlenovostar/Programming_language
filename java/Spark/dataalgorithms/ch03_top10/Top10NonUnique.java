...
public class Top10NonUnique {
   ...
   public static void main(String[] args) throws Exception {
      ...
      lines.saveAsTextFile("hdfs://localhost:9000/spark/output/1");

      /*
      * What is the right number of partitions? There is no magic bullet for calculating
      * the number of partitions. It depends on the number of cluster nodes, the number of cores
      * per server, and the size of RAM available. My experience indicates that you need to set
      * this through trial and error. The general rule of thumb is to use (2 × num_executors ×
      * cores_per_executor) per executor.
      *
      * 这种数据 partitions 的时候，Spark应该是根据hash值进行切分，因此同样的key就到了同一个partitions.
      * */
      // STEP-5: partition RDD 
      // public JavaRDD<T> coalesce(int numPartitions)
      // Return a new RDD that is reduced into numPartitions partitions.  
      JavaRDD<String> rdd = lines.coalesce(4);
      ...
      kv.saveAsTextFile("hdfs://localhost:9000/spark/output/2");
      ...
      uniqueKeys.saveAsTextFile("hdfs://localhost:9000/spark/output/3");
      ...
      * */
      /*
      * JavaRDD mapPartitions(FlatMapFunction<Iterator<T>,U> f)
      ...
      * */
      ...
      partitions.saveAsTextFile("hdfs://localhost:9000/spark/output/4");
